...
* CloudFront serves content using compression as below
	...
	8. CloudFront determines whether the file is compressible:
		...
		a. file must be of a type that CloudFront compresses.

		b. file size must be between 1,000 and 10,000,000 bytes.

		c. response must include a Content-Length header for CloudFront to determine the size within valid compression limits. If the Content-Length header is missing, CloudFront won’t compress the file.

		d. value of the Content-Encoding header on the file must not be gzip i.e. the origin has already compressed the file.
